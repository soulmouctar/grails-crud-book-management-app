...
class LivreControllerSpec extends Specification implements ControllerUnitTest<LivreController>, DomainUnitTest<Livre> {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.livreService = Mock(LivreService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.livreService = Mock(LivreService) {
            1 * save(_ as Livre) >> { Livre livre ->
                throw new ValidationException("Invalid instance", livre.errors)
            }
        }
